...
# Views Administrativas para Tipos de Relatórios

@login_required
def admin_tipos_relatorios(request):
    """Lista todos os tipos de relatórios para administração."""
    if not request.user.is_staff:
        messages.error(request, 'Acesso negado. Apenas administradores podem acessar esta área.')
        return redirect('accounts:dashboard')
    
    tipos = TipoRelatorio.objects.all().order_by('nome')
    
    context = {
        'tipos_relatorios': tipos,
        'title': 'Administração - Tipos de Relatórios',
    }
    return render(request, 'relatorios/admin/tipos_relatorios.html', context)


@login_required
def admin_criar_tipo_relatorio(request):
    ...
    if request.method == 'POST':
        ...
        incluir_fotos = request.POST.get('incluir_fotos') == 'on'
        incluir_medidas = request.POST.get('incluir_medidas') == 'on'
        incluir_frequencia = request.POST.get('incluir_frequencia') == 'on'
        ativo = request.POST.get('ativo') == 'on'
        
        if nome:
            tipo = TipoRelatorio.objects.create(
                nome=nome,
                descricao=descricao,
                template_filename=template_filename,
                incluir_graficos=incluir_graficos,
                incluir_fotos=incluir_fotos,
                incluir_medidas=incluir_medidas,
                incluir_frequencia=incluir_frequencia,
                ativo=ativo
            )
            messages.success(request, f'Tipo de relatório "{tipo.nome}" criado com sucesso!')
            return redirect('relatorios:admin_tipos_relatorios')
        else:
            messages.error(request, 'Nome é obrigatório.')
    
    context = {
        'title': 'Criar Tipo de Relatório',
    }
    return render(request, 'relatorios/admin/criar_tipo_relatorio.html', context)


@login_required
def admin_editar_tipo_relatorio(request, pk):
    """Editar tipo de relatório existente."""
    if not request.user.is_staff:
        messages.error(request, 'Acesso negado. Apenas administradores podem acessar esta área.')
        return redirect('accounts:dashboard')
    
    tipo = get_object_or_404(TipoRelatorio, pk=pk)
    
    if request.method == 'POST':
        tipo.nome = request.POST.get('nome', tipo.nome)
        tipo.descricao = request.POST.get('descricao', tipo.descricao)
        tipo.template_filename = request.POST.get('template_filename', tipo.template_filename)
        tipo.incluir_graficos = request.POST.get('incluir_graficos') == 'on'
        tipo.incluir_fotos = request.POST.get('incluir_fotos') == 'on'
        tipo.incluir_medidas = request.POST.get('incluir_medidas') == 'on'
        tipo.incluir_frequencia = request.POST.get('incluir_frequencia') == 'on'
        tipo.ativo = request.POST.get('ativo') == 'on'
        
        tipo.save()
        messages.success(request, f'Tipo de relatório "{tipo.nome}" atualizado com sucesso!')
        return redirect('relatorios:admin_tipos_relatorios')
    
    context = {
        'tipo': tipo,
        'title': f'Editar Tipo de Relatório - {tipo.nome}',
    }
    return render(request, 'relatorios/admin/editar_tipo_relatorio.html', context)


@login_required
def admin_excluir_tipo_relatorio(request, pk):
    ...
    if request.method == 'POST':
        ...
        try:
            tipo.delete()
            messages.success(request, f'Tipo de relatório "{nome}" excluído com sucesso!')
        except Exception as e:
            messages.error(request, f'Erro ao excluir tipo de relatório: {str(e)}')
        
        return redirect('relatorios:admin_tipos_relatorios')
    
    context = {
        'tipo': tipo,
        'title': f'Excluir Tipo de Relatório - {tipo.nome}',
    }
    return render(request, 'relatorios/admin/excluir_tipo_relatorio.html', context)
