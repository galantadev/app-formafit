...
class FaturaForm(forms.ModelForm):
    ...
    class Meta:
        model = FaturaSimples
        fields = ['aluno', 'mes_referencia', 'ano_referencia', 'valor', 'data_vencimento', 'observacoes']
        labels = {
            'aluno': 'Aluno',
            'mes_referencia': 'Mês',
            'ano_referencia': 'Ano',
    ...
    def __init__(self, user, *args, **kwargs):
        ...
        if self.instance.pk:
            ...
            self.fields['mes_referencia'].disabled = True
            self.fields['ano_referencia'].disabled = True
    ...
    def clean(self):
        ...
        if aluno and mes_referencia and ano_referencia:
            # Verificar se já existe uma fatura para este aluno no mesmo mês/ano
            existing_fatura = FaturaSimples.objects.filter(
                aluno=aluno,
            ...
            if self.instance.pk:
                existing_fatura = existing_fatura.exclude(pk=self.instance.pk)
            
            if existing_fatura.exists():
                raise forms.ValidationError(
                    f'Já existe uma fatura para {aluno.nome} no mês {mes_referencia}/{ano_referencia}.'
                )
