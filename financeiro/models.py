...
class FaturaSimples(models.Model):
    """
    Modelo simplificado para faturas mensais dos alunos.
    """
    STATUS_CHOICES = [
        ('pendente', 'Pendente'),
        ('atrasada', 'Atrasada'),
    ...
    data_pagamento = models.DateField(
        blank=True, 
        null=True,
        help_text="Data do pagamento (quando pago)"
    )
    observacoes = models.TextField(
        blank=True,
        help_text="Observações sobre a fatura"
    )
    data_criacao = models.DateTimeField(auto_now_add=True)
    data_atualizacao = models.DateTimeField(auto_now=True)
    ...
    def __str__(self):
        return f"{self.aluno.nome} - {self.mes_nome}/{self.ano_referencia} - R$ {self.valor}"
    
    def save(self, *args, **kwargs):
        """Atualiza automaticamente o status baseado na data de vencimento."""
        if self.status != 'paga':
    ...
    @property
    def dias_atraso(self):
        """Calcula quantos dias de atraso a fatura possui."""
